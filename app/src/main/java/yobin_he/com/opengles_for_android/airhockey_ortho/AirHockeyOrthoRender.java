package yobin_he.com.opengles_for_android.airhockey_ortho;
...
//        };
    ...
    public void onSurfaceChanged(GL10 gl, int width, int height) {
        ...
        final  float aspectRatio = width > height ?
                (float) width / (float) height : (float) height / (float) width; //用于确定相应的宽高比（>1）

        //其中project用于储存矩阵坐标，从而创建正交投影矩阵。(确保是opengl.Matrix)
        if(width > height){ // 横向屏幕
